...
public class FaucetJavaCompilerFactory implements JavaCompilerFactory {
    ...
    public static JavaCompilerFactory createFaucetCompilerFactory(WorkerDaemonFactory workerDaemonFactory, JavaForkOptionsFactory forkOptionsFactory, WorkerDirectoryProvider workerDirectoryProvider, ExecHandleFactory execHandleFactory, AnnotationProcessorDetector processorDetector, ClassPathRegistry classPathRegistry, ActionExecutionSpecFactory actionExecutionSpecFactory) {
        return new FaucetJavaCompilerFactory(workerDirectoryProvider, workerDaemonFactory, forkOptionsFactory, execHandleFactory, processorDetector, classPathRegistry, actionExecutionSpecFactory);
    }
    ...
    private <T extends CompileSpec> Compiler<T> createTargetCompiler(Class<T> type) {
        ...
        if (CommandLineJavaCompileSpec.class.isAssignableFrom(type)) {
            throw new IllegalArgumentException("Faucet does not currently support using the command line compiler.");
        }
